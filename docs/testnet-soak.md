...
- [ ] Alert delivery verified end-to-end: `python scripts/test_alert.py --config /etc/gridbot/gridbot.yaml` (sends INFO/WARNING/CRITICAL test messages) and confirm the ones marked "expect delivery" actually land in the chat/channel, not just a clean exit (delivery failures log an `ERROR`, they don't raise).
