...
- **ATR proxy**, computed from minute-level candles (or synthetic candles from trade stream). Used for breakout thresholds, anchor shift detection, and grid step/range scaling. Floored at 25 bps of mid price: minute candles built only from trade ticks can collapse the raw true range toward zero on a sparse minute, which would turn the breakout-distance check and the momentum micro-filter into hair triggers on ordinary noise, and shrink the Core range (`2.5 x ATR`) below one grid step so the grid degenerates to a single level. 25 bps keeps the Core range spanning several steps without distorting a normally active market.
...
- Compare REST snapshot against local state, **by exchange oid, not by client order id.** Client order ids are deterministic (section 7.3), so the local view can legitimately hold two orders under one cloid (a live order plus a not-yet-dropped cancelled one); a cloid-set comparison reads that as agreement and never clears the phantom, which then loops in the cancel diff. The oid set is the identity that matters.
...
*Open-orders ownership (2026-09-10):* `MarketData` holds the authoritative open-orders view (`symbol -> {oid: OpenOrder}`). The `orderUpdates` handler maintains it directly, add on `open`, drop on `filled`/`canceled`/`rejected`, adjust `remaining` on a partial, and `set_open_orders` re-bases it from REST truth (this reconciliation path, restart recovery, post-flatten reset). All mutations take `MarketData._lock`, which is the serialisation point this section requires between the WS processor task and the Supervisor's REST reconciliation. The Supervisor reads a fresh snapshot each cycle (`get_open_orders`, returns copies). The placed-cloid guard (section 7.3) still covers the brief window between OrderManager placing an order and its `open` echo arriving on the WS.
...
Each level's order size is the per-level share of this asset's inventory
cap, scaled inversely to realized volatility:

```
per_level_budget = max_abs_position / levels_per_side   # Expansion: / expansion_levels_per_side
order_size = per_level_budget * (baseline_vol / realized_vol)
order_size = clamp(order_size, min_size, per_level_budget)
```

| Parameter | Description |
|---|---|
| `per_level_budget` | `max_abs_position` (derived, section 9.1) divided across the layer's levels. This is the size at the vol baseline and the upper clamp. |
| `baseline_vol` | Trailing median of the realized-vol history (`RiskManager.get_baseline_vol`); a fixed fallback (0.5) until the history is deep enough. |
| `realized_vol` | Current annualized realized volatility |
| `min_size` | Exchange minimum order size (one lot, hard floor) |

**Reasoning:** In high volatility, each fill carries more directional risk per dollar. Scaling below the per-level budget keeps the dollar-risk-per-fill consistent. At or below the baseline, size sits at the full per-level budget (the clamp is the low-vol defense from the note below). Keying the target to `max_abs_position` rather than a raw fraction of equity keeps sizing consistent with the inventory cap, so the vol term actually modulates size between the min lot and the budget instead of always pinning to a clamp bound.

**Low-vol interaction with grid step:** In low-volatility regimes, the ATR-based grid step shrinks (toward the fee floor) while vol-scaled order size increases (toward `per_level_budget`). The combination, tighter spacing with larger orders, means more levels fill in a smaller price range with larger positions. If volatility then spikes, the bot may already be near the inventory cap before the vol circuit breaker fires. The `per_level_budget` clamp is the primary defense, and because it is `max_abs_position / levels_per_side`, the full grid filling on one side lands exactly at the cap rather than beyond it. The fee floor on grid step (section 5.4) provides a secondary bound by preventing arbitrarily tight spacing. The ATR proxy also has a floor (section 4.2) so the step cannot collapse to the point where `2.5 x ATR` spans only one level.
...
**Grid feasibility check (2026-09-10):** the pre-flight also verifies that `min_lot * levels_per_side <= max_abs_position` (the derived cap, section 9.1). If the exchange minimum lot times the level count exceeds the cap, the first few min-lot fills exhaust the cap and every remaining level is reduce-only from the start, the grid can never express its own vol-scaled sizing or inventory skew, and it degenerates to roughly one level that churns on sub-tolerance price drift. The violation message names the maximum feasible `levels_per_side` and the levers that raise the cap (equity, `capital_allocation`, `leverage`, asset weight). Like the other pre-flight checks, this is a hard gate with no override.
...
**Bootstrap window and persistence (2026-08-20):** Both thresholds are percentiles of trailing 7d vol, which is meaningless with under 48h of history, see the vol-history bootstrap decision (48h minimum, expanding to 7d, tightened pause threshold during bootstrap). This window is now persisted to `StateStore` (raw observed samples, no interpolation) and reloaded on restart, rather than living only in `RiskManager` memory, a process restart no longer discards real, already-accumulated history.

**Bootstrap sufficiency is coverage-based (2026-09-10):** an earlier revision required a single trailing run of samples with no internal gap over 5 minutes; on a multi-week testnet soak this reset the entire 48h bootstrap on any single blip (a deploy, a transient network drop, a scheduled-maintenance window), so the grid essentially never came online. Sufficiency now measures *coverage*: the trailing 48h is split into 5-minute buckets, a bucket counts as covered if it holds at least one real sample, and the window is sufficient when the oldest sample is ≥ 48h old and ≥ 80% of buckets are covered. Scattered short gaps subtract only their own duration; a multi-hour outage still drops coverage below the threshold and forces a fresh bootstrap. The 48h→7d threshold-tightening ramp is likewise driven by covered time, not wall-clock span.

**No synthetic samples in the distribution (2026-09-10):** `MarketData` returns a conservative fallback realized-vol (100% annualized) when there are too few trades or too short a span to measure. That fallback is still used for order sizing and the grid slippage buffer, but it is flagged (`VolMetrics.realized_vol_valid = False`) and is **not** recorded into the vol-history percentile distribution, and the vol circuit breakers and regime signal 1 skip the cycle rather than classify a fabricated value. Previously every trade-sparse cycle wrote a 1.0 into the history, producing a bimodal distribution that made every percentile untrustworthy in whichever direction happened to hurt.

*Note (2026-08-24):* only `vol_pause_threshold` tightens during the 48h-7d ramp (70th percentile instead of the configured default, per the original bootstrap decision). `vol_kill_threshold` stays at its configured value throughout: the <48h gate already prevents it firing on a cold start, and tightening it further during 48h-7d had it collapsing down toward the tightened pause value, erasing the intended severity gap between "stop new orders" and "cancel and flatten" right when history is thinnest and a moderate vol reading would trigger a full flatten instead of a pause.
...
if remaining > 0:
    ...
    protect the residual with a backstop trigger order (§6.8) before entering DEAD,
    the standing backstop was already cancelled ahead of the flatten attempt
...
**Not a grid-diff orphan (2026-09-10):** the backstop trigger order is never part of `GridEngine`'s desired set, its lifecycle (steps 1-4 above) is owned entirely by `OrderManager.reconcile_with_backstop`. Once the open-orders view could include it (section 4.3's WS-primary fix), a naive grid diff would read a resting backstop as "not in desired" and cancel it as an orphan every cycle, racing this section's own cancel-and-replace and effectively pinning the backstop in a permanent churn loop. `reconcile_with_backstop` computes this config's backstop cloids up front and excludes them from the grid diff; a backstop left over from a *stale* config hash (e.g. after a re-anchor) is not excluded, so it still falls through to the ordinary orphan-cancel safety net.

**Unconditional replace-every-cycle (2026-09-10):** independent of the above, step 2's cancel-and-replace previously ran on *every* reconcile a position was open, not only when the backstop actually needed to change. `trigger_price` is a function of live ATR, which is recomputed from the trade stream every cycle and drifts by a small amount continuously, so the backstop was cancelled and re-placed dozens of times a minute even with a perfectly static position, visible on the exchange UI as the stop-loss constantly disappearing and reappearing. Cancel and place are separate, non-atomic API calls (section 6.7's flatten uses the same constraint), so this needlessly widened the dead-man's-switch's absence window on every cycle for no functional reason. Fixed: before cancelling, compare the resting backstop (if any, matched by cloid against the open-orders view) to the newly computed trigger price and size using the same tolerance bands the grid reconcile uses (`reconcile_price_tolerance_bps`, `reconcile_size_tolerance_pct`, section 7.2); skip the cancel+replace entirely when it already matches.
...
The stale-view cache guards one lag direction (a cancel the view hasn't shown gone). The other direction is a *placement* the view hasn't shown yet: right after a successful place, WS `orderUpdates` for the new order can trail by a cycle, so the next diff sees the level as un-placed and places it again, resting two orders on one cloid (observed live: the grid had shrunk to one level per side that jittered around the tolerance band, and the cancel-and-replace raced its own WS confirmation). The OrderManager therefore also keeps a short-lived positive cache: `cloid -> (oid, timestamp)` for every order it has placed but not yet seen back in the reconcile view. A desired order whose cloid is in that cache is deferred; the entry clears as soon as its oid appears in `current` (WS or the 5s REST rebuild), or after 30s (an entry that outlives that almost certainly means the placement failed, and a retry is safer than blocking the level forever).
...
**Lifecycle implementation (2026-09-10):** removal was specified but never implemented, so pending flips only ever accumulated and a flip from a long-closed position was re-placed every cycle forever (and once price crossed its level, every placement was a Post-Only rejection). Now: (1) a fill whose `client_order_id` matches a pending flip's deterministic cloid (`types.flip_client_order_id`, keyed on the originating fill so it is stable across re-anchoring) removes that flip and does **not** chain a counter-flip, the originating grid level is re-placed by normal reconciliation; (2) restart recovery (section 4.4) prunes any pending flip the exchange-confirmed position can no longer back, a flip reduces exposure on one side, so flips whose reducing direction has no remaining inventory behind it (or exceeds it in aggregate size) are orphans from a position that was unwound while the bot was down; (3) `CANCEL_AND_FLATTEN` and `KILL` clear the whole pending-flips set once the position is force-closed (the "unwound by breakout flatten" case) so it cannot re-place a naked order on the next RANGE cycle.

An **externally cancelled** flip (manual, exchange-side, maintenance) is not one of these cases: the `PendingFlip` record persists, so the next reconcile re-emits it in the desired set and re-places it. With the WS-primary open-orders view (section 4.3) that re-placement happens within a cycle of the cancellation rather than waiting on the REST interval.

**Open:** a pending flip whose position is still legitimately open but whose price the market has run past can never fill as Post-Only, and is retried (and rejected) every reconcile cycle. Options are to convert it to a reduce-only order that realises the position, or to drop it and let inventory skewing unwind the position at a worse price. Not yet decided; both trade the flip's original profit target for a bounded loss.
...
### 12.5 Fourth Design Review, Testnet Soak Findings (2026-09-10)

A multi-week testnet soak opened only 2 positions total. Root cause was not the strategy but the vol-history bootstrap gate, which almost never reached "sufficient," so regime stayed `UNKNOWN` and the grid never established an anchor.

| # | Type | Finding | Change Made | Reasoning |
|---|---|---|---|---|
| 22 | **Bug** | **Bootstrap sufficiency too fragile.** `_continuous_run` required one unbroken trailing run of samples with no internal gap > 5 min. Any single blip (deploy, transient WS drop, maintenance window) discarded the entire accumulated 48h and restarted the clock. Over weeks the bot was almost always re-bootstrapping. | Replaced with coverage-based sufficiency (§6.4): trailing 48h split into 5-min buckets, sufficient when oldest sample ≥ 48h old and ≥ 80% of buckets hold a real sample. The 48h→7d tightening ramp is driven by covered time, not wall-clock span. | Scattered short gaps should cost their own duration, not the whole bootstrap. A genuine multi-hour outage still drops coverage below threshold and forces a fresh bootstrap, which is the case the original rule was actually protecting against. |
| 23 | **Bug** | **Synthetic vol samples poisoned the percentile distribution.** `MarketData` returns a 100%-annualized fallback when trades are too sparse to measure vol; every such cycle was recorded into the vol history. On a quiet testnet the history became a bimodal mix of real ~0.3 values and 1.0 fallbacks, making every percentile (regime signal 1, `vol_pause`/`vol_kill`) untrustworthy. | `_compute_realized_vol` returns `None` when it cannot measure; `VolMetrics.realized_vol_valid` carries the flag. The fallback is still used for sizing and the grid slippage buffer, but is not recorded into the history, and the vol circuit breakers + regime signal 1 skip the cycle instead of acting on it. | The fallback is a safe input for "size smaller / quote wider," but it is not a data point about the market and must not shape a distribution that risk decisions are read from. |
| 24 | **Bug** | **Degenerate ATR proxy made breakout and momentum hair triggers.** Minute candles are built only from trade ticks; a sparse minute yields near-zero true range, collapsing the ATR proxy. `breakout_atr_distance * ATR` then fires on any small move, the momentum micro-filter suppressed new entries on sub-2-bps ticks, and `2.5 x ATR` Core range shrank below one grid step. | Floored the ATR proxy at 25 bps of mid price (`_MIN_ATR_FRAC`, §4.2). Initially set to 10 bps, raised to 25 after the soak showed 10 bps still produced a 1-level Core grid. | 25 bps sits at the low end of a realistic 1-minute BTC/ETH range, so it does not distort a normally active market, but keeps the Core range spanning several `grid_step_bps_min` steps instead of one. |
| 25 | **Bug** | **Pending-flip removal (§7.6) was never implemented.** Flips were appended on every fill and never removed. After the bootstrap fixes let the grid come online, restart recovery restored flips from long-closed positions (accumulated over many restart epochs); price had crossed two of them, so every reconcile cycle re-emitted them as Post-Only rejections. | Implemented the §7.6 lifecycle: a fill on a flip's own cloid removes the pending flip without chaining a counter-flip; restart recovery prunes flips the exchange-confirmed position can't back. Flip cloid derivation centralised in `types.flip_client_order_id`. | The design already specified removal "when the flip order itself fills, or the underlying position is unwound"; only the code was missing. Position-based pruning on recovery is the principled interpretation of "unwound by other means" for the restart case. |
| 26 | **Bug** | **Vol-scaled order sizing was unit-broken.** `_compute_order_size` computed a USD risk amount (`capital_allocation * equity / levels`) then divided by realized vol and returned the result as a contract quantity, no `/ price`. The number was always outside the `[min_lot, cap/levels]` clamp, so vol-scaling never actually modulated size. | Sizing keyed to `per_level_budget = max_abs_position / levels_per_side` scaled by `baseline_vol / realized_vol` (§5.5). `baseline_vol` (`RiskManager.get_baseline_vol`, median of the vol history) is now wired into `VolMetrics.baseline_vol`, which the grid slippage buffer already expected but never received. | Keying the target to the derived cap keeps sizing consistent with inventory limits and puts the vol term back in play between the min lot and the per-level budget. |
| 27 | **Bug** | **A too-small account ran a structurally over-committed grid silently.** With ~1000 USDC of testnet equity the derived `max_abs_position` (~0.0016 BTC) was barely one min lot, yet the bot placed a 10-25 level grid of min lots. Two same-side fills tripped `REDUCE_ONLY`; the grid could never size or skew. Nothing flagged it. | Pre-flight now fails when `min_lot * levels_per_side > max_abs_position` (§6.1), naming the max feasible level count. Testnet config raised `total_risk_budget_pct` to 0.50 and cut `levels_per_side` to 6 so the soak grid is feasible. | The other pre-flight checks bound loss and leverage; none checked that the grid can physically fit inside its own cap. A hard gate is consistent with "the math either works or it doesn't." |
| 28 | **Bug** | **Duplicate resting orders on one deterministic cloid.** The stale-view cache (entry from 76fe391) only guarded against a lagging *cancel*. A lagging *placement*, WS not yet showing an order the bot just placed, made the next diff place the same cloid again, resting two orders under it and then looping on failed cancels of the orphan. | Added a positive `cloid -> (oid, ts)` cache of placed-but-unconfirmed orders (§7.3); a desired order whose cloid is in it is deferred until the oid shows in the view or the entry expires (30s). | Symmetric with the negative dead-oid cache: both lag directions of the WS-primary view now defer a re-issue rather than acting on a stale picture. |
| 29 | **Bug** | **REST reconciliation couldn't clear a phantom duplicate.** `_rest_reconciliation` compared local vs exchange by client-order-id *set*. Once the local open-orders list held a live order plus a cancelled phantom under one deterministic cloid, `local_cloids == rest_cloids`, so it declared no divergence and never adopted REST truth. The phantom stayed in `state.open_orders` indefinitely, re-entering the cancel diff every cycle and flapping against the 60s dead-oid TTL. | Compare by exchange oid set instead (§4.3). | Exchange is truth; the oid is the identity that matters. This makes a drifted local list self-heal within one `reconcile_interval` instead of never. |
| 30 | **Bug** | **WS `orderUpdates` never wrote the local open-orders list.** The handler fed fill/position tracking but not the open-orders view, so that view was REST-only and up to `reconcile_interval` (5s) stale. Combined with deterministic cloids this was the source of the phantom duplicates in entries 28-29: any drift between REST cycles let the diff re-place a live order. | `MarketData` now owns the authoritative open-orders view (`{oid: OpenOrder}` per symbol), maintained from `orderUpdates` (add/drop/adjust-remaining) under its existing lock, re-based from REST via `set_open_orders` (§4.3). Supervisor reads a fresh copy each cycle. | The design (§4.3) always specified WS-primary with REST as backup; the open-orders half was just never wired. Single owner + single lock is the serialisation the section already required. |
| 31 | **Bug** | **`CANCEL_AND_FLATTEN` / `KILL` didn't clear pending flips.** §7.6 lists "breakout flatten" as a removal trigger, but the terminal risk actions only cleared orders, not `state.pending_flips`. After a breakout flattened the position to zero, the stale flips were re-placed on the next RANGE cycle as naked orders that, if filled, opened an unintended position. | Both terminal branches now clear `state.pending_flips` (and the WS open-orders view) after the cancel-all + flatten, via `_clear_orders_and_flips`. | The position is being force-closed; every pending flip was an unwind for a position that no longer exists. Restart recovery's position-based pruning (entry 25) only covered the restart case. |
| 32 | **Bug** | **Entry 30's WS-primary open-orders view exposed a latent backstop/grid-diff conflict.** The backstop trigger order (§6.8) was never part of `GridEngine`'s desired set. While the open-orders view was REST-only (up to 5s stale), the exchange's `frontend_open_orders` snapshot apparently didn't reliably surface it in the same cycle the grid diff ran, so the conflict was rare. Once the view updated every cycle from `orderUpdates` (which does emit lifecycle events for trigger orders), the grid diff read the resting backstop as an orphan on essentially *every* cycle and cancelled it, racing `reconcile_with_backstop`'s own cancel-and-replace under the same cloid, an unbounded churn loop that also spammed exchange-side cancel/place calls. | `reconcile_with_backstop` excludes this config's backstop cloids from the set it diffs against `desired` (§6.8 "Not a grid-diff orphan"). A stale-config-hash backstop is still swept by the ordinary orphan path. | The grid diff and the backstop lifecycle are two different owners of order state that happened to share one list; the fix draws that boundary explicitly instead of relying on REST staleness to keep them from colliding. |
| 33 | **Bug** | **A failing extra (backstop) cancel was invisible in logs.** `_parse_cancel_result`'s per-order loop stopped at `len(cancels)` (the grid-only cancel list), so a backstop cancel's status, sent in the same batch via `extra_cancel_oids`, fell off the end silently. Its outcome was only visible as an unattributed unit in the aggregate `"N succeeded, M failed"` line, no oid, no error text, which is what let entry 32 run for as long as it did before showing up as anything more specific than a generic warning. | `_parse_cancel_result` now accepts `extra_cancel_oids` and logs each one's outcome (oid + error) the same as a grid cancel. | Log lines are only useful for diagnosis if they attribute the failure to something concrete; a count with no identity isn't actionable. |
| 34 | **Bug** | **Backstop cancelled and replaced on every reconcile, independent of entry 32.** With entry 32 fixed, the churn was still visible on the exchange UI as the stop-loss constantly disappearing and reappearing: `reconcile_with_backstop` cancelled and re-placed the backstop unconditionally whenever a position was open, and `trigger_price` (a function of live ATR) drifts by a tiny amount essentially every cycle. Cancel and place are separate, non-atomic calls, so this needlessly widened the dead-man's-switch's absence window dozens of times a minute for no functional reason. | Before cancelling, compare the resting backstop (matched by cloid) against the newly computed trigger price and size using the grid reconcile's own tolerance bands (§7.2); skip the cancel+replace when it already matches (§6.8 "Unconditional replace-every-cycle"). | The grid diff already treats sub-tolerance drift as a no-op for this exact reason (§7.2); the backstop path had no equivalent check and needed one for the same reason. |
| 35 | **Bug** | **A failed flatten (§6.7 step 5) left the residual position completely unprotected.** `CANCEL_AND_FLATTEN`/`KILL` call `cancel_all_orders` before attempting the flatten, which wipes the standing backstop along with the grid orders. When the flatten's own retry/escalation loop still leaves a residual (dead-state branch), the bot enters `DEAD`, and the per-cycle reconcile that would normally maintain the backstop is one of the things `DEAD` explicitly skips. The residual sat with zero server-side protection indefinitely, a direct violation of "backstop stop-losses must exist server-side for every open position." Observed live: a failed flatten on rate-limit-exhausted batch placements left a ~0.001 BTC residual naked until manual intervention. | `_run_flatten` now re-queries the post-attempt position and, on failure, calls `OrderManager.update_backstop` directly for the residual (using the still-live grid anchor/ATR) before setting `DEAD`. If anchor/ATR aren't available to compute a valid trigger price, it logs CRITICAL rather than fabricating one. | §6.8's mechanism already covers exactly this case (it's meant to protect the position when the bot's own logic has failed to act); the gap was that the failed-flatten path never invoked it, not a missing safety concept. |
| 36 | **Bug** | **`reconcile_with_backstop` re-fetched the whole account's open orders over REST on every backstop cancel/replace.** `_find_backstop_oids` called `frontend_open_orders` to locate the backstop's oid before cancelling it, even though the caller already receives `current`, the WS-primary per-symbol open-orders view (§4.3), which includes resting trigger orders. Every backstop update was spending a request on data already on hand, directly working against §2.4's finite address-level budget for a purely maker/ALO strategy that earns little of the taker volume that budget scales with. | `_find_backstop_oids` replaced with `_find_backstop_oids_in_view`, a pure lookup against the already-fetched `current` list, no REST call. `update_backstop`'s own REST-based lookup (used only on the rare failed-flatten path, entry 35) is left as-is, correctness there matters more than budget for a one-shot emergency call. | Don't pay for data the caller already handed you; this is a pure efficiency fix, no change to backstop correctness or timing. |
| 37 | **Design decision** | **Sustained request-budget exhaustion (§2.4) had no backoff: every reconcile retried the same failing batch immediately, spending more of a budget that wasn't there and spamming an ERROR line every cycle.** Observed live: ~17 minutes of continuous `Too many cumulative requests sent` batch failures, during which backstop updates also failed (`Backstop order update failed ... position may be unprotected`), widening the window a subsequent failed flatten (entry 35) landed in. | `OrderManager` detects the `Too many cumulative requests sent` marker in a batch's `"err"` response and starts a fixed passive-backoff window (`_RATE_LIMIT_BACKOFF_SECONDS`, 5s); `_submit_batch` skips sending entirely while the window is active, logging once at INFO instead of retrying into the same error. Mirrors the existing maintenance-mode passive wait (§2.5) for a different, similarly transient, non-bug exchange-side condition, and does not gate the emergency paths (flatten IOCs, `cancel_all_orders`, `update_backstop`), which bypass `_submit_batch` entirely and must keep trying regardless. | A fixed, short, bounded backoff was chosen over a smarter scheme (e.g. parsing the volume-traded figure to estimate recovery time) because HL's error gives no reliable ETA and a bounded wait is simple to reason about; if this backoff proves too short or too long in practice, revisit with real data rather than guessing further. Entry 36's request-count reduction on the backstop path is the complementary fix, fewer requests spent means the budget lasts longer before this backoff is ever needed. |
| 38 | **Bug** | **`_is_alo_rejection` never matched Hyperliquid's actual post-only rejection text, so section 7.4's nudge-and-retry never engaged for a real rejection.** It checked for `"BadAloPx"` or `"would have been filled"`; HL's real wording is `"Post only order would have immediately matched, bbo was X@Y. asset=N"`. Every real ALO cross fell into the generic `Placement failed` branch instead, skipping the nudge. Since `GridEngine.compute_desired_orders` recomputes the same static price every cycle when the anchor hasn't moved, the identical doomed order was resubmitted every reconcile (~2.5s) with no nudge and no backoff. Observed live: one grid level retried for 21+ minutes straight (492 of 508 `Placement failed` lines at one price), which is what actually exhausted the request budget behind entry 37's condition, this session's rate-limit spam traced back to this, not to the unrelated concurrent-DB-write bug (entry 39) that happened to fire at the same moment from the same triggering fill. | `_is_alo_rejection` now also matches `"would have immediately matched"` (case-insensitively). The fabricated test string that never matched production wording is kept alongside the corrected one, not replaced, in case HL's wording varies by rejection path. | An error-text contract that was never checked against the real exchange response is worse than no contract, it gives false confidence that section 7.4's retry path is exercised when it silently never was. |
| 39 | **Bug** | **`StateStore` had no lock serializing writes across the two concurrent asyncio tasks (`Supervisor._fill_pump` and the main per-symbol cycle loop) that share one connection.** `save_open_orders` and `save_pending_flips` both run explicit multi-statement transactions (`BEGIN` .. `commit`/`rollback`); each `await conn.execute(...)` yields control back to the event loop, so the other task's `BEGIN` could land while one was still open. Observed live: `_fill_pump` -> `save_pending_flips` -> `sqlite3.OperationalError: cannot start a transaction within a transaction`, right after a flip-closing fill (the in-memory `pending_flips` removal still succeeded; only that one disk write was lost). | Added `StateStore._write_lock` (`asyncio.Lock`), held for the full body of every write method (`save_*`, `delete_grid_config`, `record_fill`, `update_heartbeat`, `append_vol_sample`). Read-only `load_*`/`get_*` methods are single-statement `SELECT`s with no transaction to protect and are left unlocked. | Two coroutines sharing one DB-API connection must never have overlapping multi-statement transactions; a single lock around every write is simpler and more robust than trying to reason about which specific method pairs can collide. Verified the fix actually closes the race (not just superficially quiets the symptom) by reproducing the exact `OperationalError` with the lock swapped for a no-op before confirming the real lock prevents it. |
| 40 | **Bug** | **Graceful shutdown (§4.5) declared `"Shutdown complete"` even when it never confirmed orders were actually cancelled.** `cancel_all_orders` made one `bulk_cancel` attempt with no retry and no post-cancel read-back; `Supervisor._shutdown` caught any exception from it, logged, and moved on regardless. Observed live: the exchange returned `ServerError(500, 'null')` on every request for ~2 minutes (the same outage that had already fired the consecutive-error KILL switch); the single shutdown cancel attempt hit that same error, and the process exited claiming a clean shutdown while a position and 4 grid orders stayed live on the exchange with no process left to run the backstop. A secondary contributor: `_submit_batch`'s batch-place exception handler assumed a raised `bulk_orders` call meant no order reached the exchange at all, but the log showed `current=0` confirmed on every reconcile during the failure window, and moments later `cancel_all_orders` found real orders to cancel, meaning at least one "failed" placement had landed server-side despite the exception. | `cancel_all_orders` now loops (bounded, 5 attempts, 2s pause) re-reading the exchange's open orders each time and returns `True` only once the exchange itself confirms zero remain for the symbol, `False` otherwise. `Supervisor._shutdown` acts on that: fires a CRITICAL alert naming any symbol it couldn't confirm, and logs `"Shutdown complete WITH UNCANCELLED ORDERS"` instead of a bare `"Shutdown complete"`. The batch-place exception handler now checks (via a fresh `open_orders` read matched by cloid) whether the attempt actually landed before logging, still re-raising either way so it still counts toward the kill switch. | A cancel or place call raising is not proof the exchange never applied it, only that the response round-trip failed; treating a raised exception as "definitely didn't happen" without checking exchange state is the same class of mistake as the KILL/flatten interaction (entry 35), trusting local reasoning over exchange truth in exactly the place §4.5 and the invariants section say not to. |

**Still open after this review (flagged, not fixed):** the `UNKNOWN`-during-bootstrap policy still blocks all trading for the full 48h even when signals 2 (price vs EMA-50) and 4 (no recent breakout) are computable and clear; the post-breakout cooldown overlaps the regime signal-4 window and elevated post-breakout vol, so real re-entry is well beyond `cooldown_minutes`; the trade buffer that feeds `_compute_realized_vol` is never time-trimmed, so the measurement is a long-horizon average rather than current vol; the single-level grid on a small account still cancel-and-replaces its lone level on sub-tolerance price drift (entry 28 stops the *duplicate*, not the churn); and a pending flip whose position is still open but whose price the market ran past is retried as Post-Only every cycle (see §7.6 Open). These need deliberate design decisions, not contained fixes.
