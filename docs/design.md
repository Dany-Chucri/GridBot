...
- **ATR proxy**, computed from minute-level candles (or synthetic candles from trade stream). Used for breakout thresholds, anchor shift detection, and grid step scaling. Floored at 10 bps of mid price: minute candles built only from trade ticks can collapse the raw true range toward zero on a sparse minute, which would turn the breakout-distance check and the momentum micro-filter into hair triggers on ordinary noise.
...
| 24 | **Bug** | **Degenerate ATR proxy made breakout and momentum hair triggers.** Minute candles are built only from trade ticks; a sparse minute yields near-zero true range, collapsing the ATR proxy. `breakout_atr_distance * ATR` then fires on any small move, and the momentum micro-filter suppressed new entries on sub-2-bps ticks. | Floored the ATR proxy at 10 bps of mid price (`_MIN_ATR_FRAC`, §4.2). | 10 bps sits at the low end of a realistic 1-minute BTC/ETH range, so it does not distort normal operation, but it prevents the proxy from decaying to a value that makes every downstream threshold trivial to cross. |
...
**Still open after this review (flagged, not fixed):** the `UNKNOWN`-during-bootstrap policy still blocks all trading for the full 48h even when signals 2 (price vs EMA-50) and 4 (no recent breakout) are computable and clear; the post-breakout cooldown overlaps the regime signal-4 window and elevated post-breakout vol, so real re-entry is well beyond `cooldown_minutes`; and the trade buffer that feeds `_compute_realized_vol` is never time-trimmed, so the measurement is a long-horizon average rather than current vol. These need deliberate design decisions, not contained fixes.
