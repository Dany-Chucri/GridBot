...
*Implementation gap (2026-09-10):* the WS `orderUpdates` handler updates fill/position tracking but does **not** currently write the local open-orders list, so that list is effectively REST-only and up to `reconcile_interval` stale. The oid-based comparison above makes REST self-heal a drifted list within one interval, and the placed-cloid guard (section 7.3) covers the REST-lag window right after a placement, but wiring WS cancellations directly into the open-orders list (serialized against the REST path, as this section requires) is still open.
